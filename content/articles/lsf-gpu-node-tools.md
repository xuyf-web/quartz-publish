---
title: 两个命令，看清 LSF 节点和 GPU 占用
description: 用两个短命令汇总 LSF CPU 节点与 NVIDIA GPU 的资源状态，并拆解脚本的数据来源、输出含义、适用场景和迁移注意事项
date: 2026-07-31
...
我们在服务器上提交任务前，通常需要关注一下有多少计算资源，LSF 计算节点还有多少可用核，GPU 节点上又有哪些卡处在空闲状态。

这些信息都在 `bjobs`、`lshosts`、`lsload`、`bhosts`、`nvidia-smi` 和 `ps` 里。单条命令都不难，但记住这么多命令很麻烦，而且每次都要来回查询、对齐，再做判断，就尤其需要一个易操作、界面友好的小工具来辅助。感谢这些基本命令的可参数化，让我能基于它们做一些满足我需要的定制。

我给服务器写了两个 Bash 小工具：`lsf-nodes` 汇总 CPU 计算节点，`list-gpu` 汇总 GPU 和计算进程，并设置了 `lf` 和 `lg` 两个短别名。

> [!summary]
> 本文介绍两个自写的小工具，用来集中查询 LSF 计算节点和 GPU 状态，减少在 `bjobs`、`nvidia-smi` 等命令之间来回切换

# 为什么要给系统命令再包一层

LSF 已经提供了完整的查询命令。`bjobs` 可以查看作业，`lshosts` 提供节点的静态资源，`lsload` 返回当前负载，`bhosts` 则给出调度系统看到的主机状态。

然而，默认命令并不完全满足我的需要，例如 bjobs 显示的结果会压缩 JOB NAME 栏的信息，对于名称较长的任务总会显示不全；对于 nvidia-smi 来说也有同样的问题，在 tmux 半屏中，该命令返回的结果由于宽度不足而自动换行，导致表格错位，难以阅读。

这就是我写这两个小命令的初衷：为了快速获取我需要的信息，并尽可能地美观和便捷。下文将会介绍实现方式，读者可以据此设计符合自己需要的命令，不必复刻我的程序。
...
![lsf-nodes 运行效果](../assets/两个命令，看清%20LSF%20节点和%20GPU%20占用-20260731.png)
...
![GPU 状态概览](../assets/两个命令，看清%20LSF%20节点和%20GPU%20占用-20260731-1.png)
...
![GPU 计算进程](../assets/两个命令，看清%20LSF%20节点和%20GPU%20占用-20260731-2.png)
...
完整脚本改为独立文件维护，文章只保留实现思路、字段解释和迁移时需要注意的参数。下载前请先阅读说明，并根据服务器环境修改节点前缀、GPU 主机名和共享路径。

- [[../files/lsf-nodes|下载 `lsf-nodes.sh` 并查看使用说明]]
- [[../files/list-gpu|下载 `list-gpu.sh` 并查看使用说明]]

# 写在最后

`lsf-nodes` 和 `list-gpu` 都只有一百行左右。它们没有引入新的监控服务，只是把我每次都要手动拼接的信息整理成了两张终端表格。

对这种命令行工具，我最需要的就是查询要足够快，字段要能直接支持下一步判断，脚本失败时也不会影响作业或系统状态。

如果你也经常在登录节点、CPU 集群和 GPU 节点之间切换，可以像我一样构建一套最适合你的查询命令，日常使用会高效很多。
